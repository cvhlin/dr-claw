...
If no suggested skills appear in the prompt, or the user makes a freeform request outside the task list, list the `.claude/skills/` directory to discover available skills and pick the best match. When unsure, start with `inno-research-orchestrator` — it is the main entry point that routes to other skills.
