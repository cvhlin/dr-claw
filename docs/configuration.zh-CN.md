...
VibeLab 通过项目根目录下的 `.env` 文件中的环境变量进行配置。本指南记录了应用读取的所有变量。
...
| 变量 | 是否必需 | 默认值 | 说明 |
|------|---------|--------|------|
| `JWT_SECRET` | **是**（生产环境） | `claude-ui-dev-secret-change-in-production` | 用于签名和验证 JWT 令牌的密钥。在将 VibeLab 暴露到 localhost 以外之前**必须**更改。生成方法：`node -e "console.log(require('crypto').randomBytes(32).toString('hex'))"` |
...
| `WORKSPACES_ROOT` | 否 | 用户主目录（`os.homedir()`） | VibeLab 查找和创建项目工作区的根目录。仅在 `VITE_IS_PLATFORM=true` 时有意义。 |
...
| | OSS 模式（默认） | 平台模式 |
|---|---|---|
| **适用场景** | 本地运行 VibeLab 的个人开发者 | 托管 / 多租户部署 |
...
> 在 OSS 模式下，`WORKSPACES_ROOT` 变量被忽略 — VibeLab 从用户主目录下的 Claude Code / Cursor / Codex 会话目录中发现项目。

---

## 安全检查清单

在将 VibeLab 部署到网络（而非仅 `localhost`）之前，请检查以下事项：

1. **`JWT_SECRET`** — 将默认值替换为强随机字符串。默认值是公开的，不提供任何安全保障。
2. **`API_KEY`** — 考虑设置 API 密钥以增加额外的认证层。
3. **`WORKSPACES_ROOT`** — 在平台模式下，确保此路径指向你信任的目录。VibeLab 会提供该目录树下的文件内容。
4. **`.gitignore`** — 确认 `.env` 已列入 `.gitignore`（默认已包含），防止密钥被提交。
5. **HTTPS** — 将 VibeLab 暴露到公网时，请将其放在反向代理（如 Nginx、Caddy）后面，并启用 TLS。
